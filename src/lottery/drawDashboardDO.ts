...
function parseCompetitionIdFromDoId(id: DurableObjectIdLike): number | null {
    ...
    if ('name' in id && typeof id.name === 'string') {
        ...
        if (str.includes('draw-')) {
            name = str
        } else {
            // If toString doesn't contain the prefix, it's likely a numeric ID, which we can't parse
            console.error('[DrawDashboardDO] Unable to parse competition ID from:', { id, name: id.name, toString: str })
            return null
        }
    }
    ...
    if (!name.startsWith(prefix)) {
        console.error('[DrawDashboardDO] ID name does not start with expected prefix:', { name, prefix })
        return null
    }
    ...
    if (isNaN(parsed)) {
        console.error('[DrawDashboardDO] Failed to parse competition ID number:', { name, num })
        return null
    }
...
function sseMessage(data: DrawStateResponse): string {
    return `data: ${JSON.stringify(data)}\n\n`
}

export class DrawDashboardDO extends DurableObject<Env> {
    ...
    private async handleGet(request: Request): Promise<Response> {
        ...
        const initialStateHeader = request.headers.get(INITIAL_DRAW_STATE_HEADER)
        let state: DrawStateResponse | null = null
        if (initialStateHeader) {
            try {
                state = JSON.parse(base64DecodeUtf8(initialStateHeader)) as DrawStateResponse
            } catch {
                // ignore bad header
            }
        }
        ...
        this.ctx.waitUntil(
            (async () => {
                try {
                    const resolvedState = state ?? (await getDrawState(this.env, competitionId))
                    await write(sseMessage(resolvedState))
                } catch (e) {
                    await write(sseMessage({ participantCount: 0 })).catch(() => {})
                }
                    ...
                    const scheduleNext = (): void => {
                        this.heartbeatTimer = setTimeout(async () => {
                            try {
                                ...
                                console.error('[DrawDashboardDO] heartbeat broadcast error:', err)
                            }
                            scheduleNext()
    ...
    private async handleBroadcast(request: Request): Promise<Response> {
        let body: DrawStateResponse
        try {
            body = (await request.json()) as DrawStateResponse
        } catch {
